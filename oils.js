...
window.onload = function () {
    ...
    fetch(url)
        ...
        .then(data => {
            const json = parseGoogleSheetResponse(data, sheetName);
            ...
            rows.forEach(row => {
                ...
                if ([stockAvailability, brand, thc, cbd, strain, packSize, sativaIndica, pricePG].includes("Unknown")) {
                    return;
                }

                stockData.push({
                    stockAvailability, brand, thc, cbd, strain, packSize, sativaIndica, pricePG
                });
                ...
                stockData.forEach(stock => {
                    ...
                    if ((!stockAvailabilityFilter || stock.stockAvailability.trim() === stockAvailabilityFilter.trim()) &&
                        ...
                        let row = `<tr>
                            ...
                            <td data-label="Full Spectrum/Isolate">${stock.strain}</td>
                            <td data-label="Pack Size (ML)">${stock.packSize}</td>
        ...
        .catch(error => {
            ...
            cell.textContent = `Error: Could not load data from the spreadsheet. ${error.message}`;
